function Bear() { //The main Bear class
    this.dBear = 100; //step in pixels the bear takes after user presses an arrow
    this.htmlElement = document.getElementById("bear"); //Getting the image
    ...
    this.fitBounds = function() { //without this the bear goes out of bounds. This function fits the bear within the board limits
    ...
    this.setSpeed = function() {
        let b_speed = document.getElementById("speedBear"); // get the element
        this.dBear = Number(b_speed.value);
    };
...
// to move the bear
function moveBear(e) { //We create eventhandlers for keyboard events (Up,Down,Left and Right)
...
function start() { //Instantiating the Bear object
    //create bear
    bear = new Bear();
    // Add an event listener to the keypress event.
    document.addEventListener("keydown", (event) => moveBear(event), false);
    // listen to bear speed changes and update the value
    document.getElementById("speedBear").addEventListener("change", () => bear.setSpeed());
...
class Bee { //Creates and controls instances of bee objects
    constructor(beeNumber) {
        ...
        this.display = function() {
            ...
            this.fitBounds(); //adjusts to bounds
            this.htmlElement.style.left = this.x + "px";
            this.htmlElement.style.top = this.y + "px";
            this.htmlElement.style.display = "block";
        };
...
function createBeeImg(wNum) { //creates bee image and sets it randomly
...
function isHit(defender, offender) {
    if (overlap(defender, offender)) { //check if the two image overlap
        let score = document.getElementById("hits").innerHTML; //display the new score
        if (score == 1000){
            clearTimeout(updateTimer); //clearing the setTimeout()
            alert("Game Over!");
            ...
            let newStingTime = new Date(); //newStingTime is initialized and started
            let thisDuration = newStingTime - lastStingTime; //currentDuration :is new(started now) - last(started when bear first moved)
            lastStingTime = newStingTime; 
            let longestDuration = Number(duration.innerHTML); //longestDuration is gotten from the html id
            if (longestDuration === 0) { //if 0
                longestDuration = thisDuration; //change longestDuration
            } else {
                if (longestDuration < thisDuration) 
                    longestDuration = thisDuration;
            }
            document.getElementById("duration").innerHTML = longestDuration; //put back the updated value of longestDuration into the BestDuration on html
        }
    }
}   
